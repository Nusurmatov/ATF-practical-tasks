public class Bird : IFlyable
{
    ...
    // A bird loses 10% Stamina in every 10 km. When its Stamina level is 30%, it should rest 7 hours.
    // In other words, the bird rests to eat for 7 hours after every 70 km of flying.
    ...
    public void FlyTo(Coordinate point)  
    {
        ...
        message.Append($"The bird started flying with a speed of {this.Speed} km/h ");
        ...
        while (true)
        {
            if (distance > 0) 
            {
                ...
                if (this.Stamina == 30)
                {
                    Console.WriteLine("\nThe bird reached 70 km and now it is tired and hungry. It should rest and eat for about 7 hours.");
            ...
            {
                // same logic used in the GetFlyTime method
                Console.WriteLine("\nThe bird has arrived and it took rougly {0} hours.\n", Math.Round(S/this.Speed + S/70.0 *7));  // S -distance
                break;
            }
    ...
    public int GetFlyTime(Coordinate point)
    {
        int time = 0;
        int distance = Coordinate.GetDistance(this.Currentposition, point);
        time += distance/this.Speed;  // t = S/v;
        time += distance/70 * 7;  // taking bird's Stamina into account   
